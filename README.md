...
- “波形对齐”：先放入字幕视频和原唱音源，生成波形后选择“调整字幕视频”或“调整原唱音源”。调整字幕视频时，负偏移会在导出时裁掉视频开头；正偏移会先尝试按源视频参数生成黑场并 `concat -c copy` 无损拼接，失败或检测到丢流时自动回退到重编码补黑。调整原唱音源时，负偏移会裁掉音频开头，正偏移会给音频前面补静音；导出结果统一默认为 WAV PCM 无损音频。
