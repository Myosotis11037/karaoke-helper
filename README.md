...
- 波形对齐支持两种目标：调整字幕视频，或调整原唱音源并导出修正后的音频；其中调整字幕视频时会统一重编码视频导出
...
- “波形对齐”：先放入字幕视频和原唱音源，生成波形后选择“调整字幕视频”或“调整原唱音源”。可以用“自动对齐”先估算偏移，再拖动波形上的播放头选择预览起点，同时听字幕视频音轨和原唱音源来确认是否对齐；未生成波形时按空格会生成波形，生成后按空格可以切换播放/停止，`Ctrl+D` 会自动对齐，`Ctrl+S` 会导出当前对齐目标。调整字幕视频时，负偏移会在导出时裁掉视频开头，正偏移会在视频前面补前导画面；这两种情况以及仅做尾裁时，都会统一通过重编码导出视频。调整原唱音源时，负偏移会裁掉音频开头，正偏移会给音频前面补静音；导出结果统一默认为 WAV PCM 无损音频。
- 调整字幕视频时，可以选择“软编省空间”或“硬编快速”。硬编会优先使用 NVENC（例如 `hevc_nvenc` / `h264_nvenc`），速度更快但文件可能更大；不可用时会自动改用软编。
- “Hi-Res 生成”：选择字幕视频、原唱音频、伴奏音频，并开始最终封装导出。
