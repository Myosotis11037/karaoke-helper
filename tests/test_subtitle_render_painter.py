...
import krok_helper.subtitle_render.engine.render.core.raster_blur as raster_blur  # noqa: E402
...
def test_font_builders_use_runtime_qt_family_alias(monkeypatch):
    ...
    monkeypatch.setattr(title_render, "resolve_qt_font_family", resolve_family)
    style = Style(
...
from krok_helper.subtitle_render.engine.layout.page.assignment import (  # noqa: E402
...
def test_section_ending_clear_caps_animation_restore_at_section_end(qapp):
    ...
    # 旧流程下守卫动画恢复先于填充：下句拉到 2_000+1_500 = 3_500，
    # 段尾页填充把上句也挂到页内最大 3_500；clear 把两句都钳回 3_000。
    assert hold[0][1] == hold[1][1] == 3_500
...
def test_animation_guard_compresses_exit_animation_to_minimum_in_display_window(qapp):
    ...
    # 同轨间隔 300：需要的起点 = 2_900 + 300 = 3_200，重叠 100ms 两侧
    # 各担 50——前句退场 900ms 压到 850ms、后句入场 900ms 压到 850ms，
    # 都仍高于各自下限。
...
def test_animation_guard_cuts_buffers_greedily_then_shares_animations(qapp):
    ...
    buffered = _apply_painter_animation_time_guard(
        ...
        720,
        track,
        replace(style, exit_fade_ms=100),
        [
            DisplayLine(lines[0], 0, 100, 2_180, 0, 1, 1),
            DisplayLine(lines[1], 0, 2_300, 6_000, 0, 2, 1),
        ],
        enforce_inter_page_gap=True,
    )
